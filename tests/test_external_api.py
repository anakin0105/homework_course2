...
def test_convert_to_rub_success():
    transaction: Dict = {"operationAmount": {"amount": "100", "currency": {"code": "USD"}}}
...
def test_convert_to_rub_rub_no_api():
    transaction = {
        ...
        "operationAmount": {"amount": "5000.56", "currency": {"name": "RUB", "code": "RUB"}},
    }
...
def test_convert_to_rub_invalid_currency():
    ...
    with pytest.raises(
        ValueError, match="Некорректный код валюты 'GBP'. Допустимые значения: \\['USD', 'EUR', 'RUB'\\]"
    ):
