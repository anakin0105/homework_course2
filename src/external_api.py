...
def convert_to_rub(
    transaction: dict,
) -> float:  # Определяю функцию covert, которая принимает аргумент transaction (словарь с данными транзакций)
    ...
    currency = transaction["operationAmount"]["currency"][
        "code"
    ]  # Извлекаю код валюты (например, "USD", "EUR", "RUB") из словаря transaction
    amount = float(
        transaction["operationAmount"]["amount"]
    )  # Извлекаю сумму транзакций (например, "100.00") из словаря transaction
    try:
        amount = float(amount)
        if amount < 0:
    ...
    if (
        currency == "RUB"
    ):  # Проверяю, является ли валюта рублями, если нет (например, "USD", "EUR"), нужно конвертировать
        return amount
    try:
